          ...
          <div class="tab-content" id="custom-tabs-two-tabContent">
                    ...
                    <table id="example3"class="table table-bordered">
                        ...
                        @foreach ($domisili as $no => $kat)
                            <tr>
                                ...
                                <td>
                                  @if ($kat->agama == 1)
                                  <center><span class="badge badge-info">Islam</label></center>
                                      @elseif($kat->agama == 2)
                                      <center><span class="badge badge-secondary">Kristen</label></center>
                                          @elseif($kat->agama == 3)
                                          <center><span class="badge badge-success">Hindu</label></center>
                                              @elseif($kat->agama == 4)
                                              <center><span class="badge badge-warning">Buddha</label></center>
                                                  @elseif($kat->agama == 5)
                                                <center><span class="badge badge-warning">Konghucu</label></center>
                        ...
                        {{--  <a href="/domisili/show/{{$kat->id}}" rel="noopener" target="_blank" class="btn btn-xs btn-default">
                          ...
                          <a href="/domisili/edit/{{$kat->id}}" class="btn btn-xs btn-info">
                             <i class="fa fa-check"></i> Lihat
                          </a>

                          <form method="POST" class="d-inline" onsubmit="return confirm('Masukan ke tempat sampah?')" action="/domisili/{{ $kat->id }}/destroy">
                            @csrf
                        ...
                        @foreach ($domisili_ok as $no => $kat)
                            <tr>
                                ...
                                <td>{{ $kat->updated_at }} 
                                  ...
                                  <span class="badge badge-light">{{ $kat->operator }}</span>
           ...
           <div class="card-body">
            ...
            <table id="example4"class="table table-bordered">
                ...
                <tbody>
                @foreach ($domisili_trash as $no => $kat)
                    <tr>
                        <th scope="row" style="text-align: center">{{ ++$no}}</th>
                        ...
                        <td>{{ $kat->deleted_at }} 
                          <center>
                          <span class="badge badge-secondary">{{ $kat->operator }}</span>
                        </center>
                      </td>
                      
                        <td class="text-center">
                          <form method="POST" action="/domisili/{{ $kat->id }}/restore" class="d-inline">
                            @csrf
                            <button type="submit" value="Restore" class="btn btn-xs btn-success"/>
                            Restor <i class="fas fa-undo"></i>
                        </form>
                        
                        <form method="POST" action="/domisili/{{ $kat->id }}/delete-permanent" class="d-inline" onsubmit="return confirm('
